...
get_occupied_docks(Shipping_State, Port_ID) ->
    Occupied_Docks = lists:filter(fun ({_Port_fun, _Dock_fun, _Ship_fun}) -> %% the _ before the variable makes the varaible bounded to avoid unused errors
        _Port_fun =:= Port_ID end, Shipping_State#shipping_state.ship_locations),
    lists:map(fun ({_Port_fun, _Dock_fun, _Ship_fun}) -> _Dock_fun end, %% the _ before the variable makes the varaible bounded to avoid unused errors
        Occupied_Docks).


get_ship_location(Shipping_State, Ship_ID) ->
    case lists:keyfind(Ship_ID, 3, Shipping_State#shipping_state.ship_locations) of
        {Port_get, Dock_get, _Ship_get} -> {Port_get, Dock_get}; %% the _ before the variable makes the varaible bounded to avoid unused errors
        _ -> error
    end.
...
get_container_weight(Shipping_State, Container_IDs) ->
    case Container_IDs of
        ...
        [_H | _T] ->  %% the _ before the variable makes the varaible bounded to avoid unused errors
            lists:foldl(fun (CID_fun, Sum_fun) ->  
                (get_container(Shipping_State, CID_fun))#container.weight + Sum_fun end, 0,  Container_IDs);
...
load_ship(Shipping_State, Ship_ID, Container_IDs) ->
    ...
    Ship_CCap = (get_ship(Shipping_State, Ship_ID))#ship.container_cap,
    case Container_IDs of
      [] -> Shipping_State;
      [_H | _T] ->
        {ok, SHINV} = maps:find(Ship_ID, Shipping_State#shipping_state.ship_inventory),
        {ok, Port_Inv} = maps:find(Port, Shipping_State#shipping_state.port_inventory),
        case(length(Ship_Inv) + length(Container_IDs) =< Ship_CCap) and is_sublist(Port_Inv, Container_IDs) of
          true ->
            Ship_Inv_New = lists:merge(Ship_Inv, Container_IDs),
            Port_Inv_New = lists:filter(fun (X) -> not lists:member(X, Container_IDs) end, Port_Inv),
                {ok, #shipping_state{
                        ...
                        ship_inventory = maps:put(Ship_ID ,Ship_Inv_New,Shipping_State#shipping_state.ship_inventory),
                        port_inventory = maps:put(Port, Port_Inv_New, Shipping_State#shipping_state.port_inventory)
                }};
          false ->
            error
          end;
...
unload_ship_all(Shipping_State, Ship_ID) ->
    {ok, Ship_Inv} = maps:find(Ship_ID, Shipping_State#shipping_state.ship_inventory),
      unload_ship(Shipping_State, Ship_ID, Ship_Inv).
...
unload_ship(Shipping_State, Ship_ID, Container_IDs) ->
    ...
    {ok, Ship_Inv} = maps:find(Ship_ID, Shipping_State#shipping_state.ship_inventory),
    {ok, Port_Inv} = maps:find(Port, Shipping_State#shipping_state.port_inventory),
    case(length(Port_Inv) + length(Container_IDs) =< PortCap) and is_sublist(Ship_Inv, Container_IDs) of
        true ->
            Ship_Inv_New = lists:filter(fun (X) -> not lists:member(X, Container_IDs) end, Ship_Inv),
            Port_Inv_New =  lists:append(Port_Inv, Container_IDs),
                {ok, #shipping_state{
                    ships = Shipping_State#shipping_state.ships,
                    containers = Shipping_State#shipping_state.containers,
                    ports = Shipping_State#shipping_state.ports,
